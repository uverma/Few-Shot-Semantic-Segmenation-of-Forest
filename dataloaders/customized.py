...
def getMask(label, scribble, class_id, class_ids):
    ...
    # fg_mask=fg_mask.float().mean(2)
    # bg_mask=bg_mask.float().mean(2)
    ...
    if scribble_cls_list:  # Still need investigation
        ...
        fg_scribble[:] = 0
    # bg_scribble.float().mean(2)
    # fg_scribble.float().mean(2)
    # print("bg_scribble",bg_scribble.shape)
    # print("fg_scribble",fg_scribble.shape)
...
def fewShot(paired_sample, n_ways, n_shots, cnt_query, coco=False):
    ...
    # print(paired_sample[0]['image'].shape)
    # # print(paired_sample[0]['basic_class_id'].shape)
    # print(paired_sample[0]['image_t'].shape)
    # print(paired_sample[0]['scribble'].shape)
    # print(paired_sample[0]['inst'].shape)
    # print(paired_sample[0]['label'].shape)
...
# torch.Size([128, 128])
    ...
    support_images_t = [[paired_sample[cumsum_idx[i] + j]['image_t'] for j in range(n_shots)]
                        for i in range(n_ways)]
    # print(support_images[0][0].shape)
    # print(support_images[0][0].shape)
