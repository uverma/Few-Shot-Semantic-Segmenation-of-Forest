...
def main(_run, _config, _log):
    ...
    if not False:
        model.load_state_dict(torch.load('/home/gokul/Documents/Btech_Hons/PANet/runs/PANet_VOC_align_sets_0_1way_5shot_[train]/4/snapshots/30000.pth', map_location='cpu'))
    model.eval()
    ...
    with torch.no_grad():
        for run in range(5):
            ...
            for sample_batched in tqdm.tqdm(testloader):
                ...
                suffix = 'scribble' if False else 'mask'
                ...
                if False:
                    ...
                    support_bg_mask = [[shot[f'bg_{suffix}'].float().cuda() for shot in way]
                                       for way in sample_batched['support_mask']]
                    # cv2.imshow('fg', support_fg_mask[0])
                    # cv2.waitKey(3000)
                    # cv2.imshow('bg', support_bg_mask[0])
                    # cv2.waitKey(3000)
